...
use examples_proto::{
    ...
    GameVariant, HandComplete, HandStarted, PotAwarded,
};
...
#[test]
fn dispatch_hand_started_returns_default_response() {
    let router = build();
    let resp = router.dispatch(pm_request(hand_started_event())).expect("dispatch ok");
    // The PM is observational - returns default responses.
