//! ReportPrematureRiver command handler.
//!
...
use examples_proto::{PrematureRiverDetected, ReportPrematureRiver};
...
pub fn handle_report_premature_river(
    _cmd: ReportPrematureRiver,
    state: &HandState,
    seq: u32,
) -> CommandResult<EventBook> {
    guard(state)?;

    let event = PrematureRiverDetected {
        detected_at: Some(angzarr_client::now()),
    };
    let event_any = pack_event(&event, "examples.PrematureRiverDetected");
    Ok(EventBook {
...
mod tests {
    ...
    fn rejects_when_hand_not_dealt() {
        ...
        let err = handle_report_premature_river(ReportPrematureRiver {}, &state, 1).unwrap_err();
    ...
    #[test]
    fn emits_premature_river_detected() {
        let state = dealt_state();
        let book = handle_report_premature_river(ReportPrematureRiver {}, &state, 11).expect("ok");
